        ...
        @else
            <div class="overflow-x-auto bg-white shadow-md rounded-lg">
                <table class="w-full border-collapse border border-gray-200">
                    ...
                    <tr>
                        <th class="text-left px-4 py-2 border border-gray-300">Ticket ID</th>
                    ...
                    @foreach($tickets as $ticket)
                        @php
                            $refund = $ticket->payment ? $refunds->where('payment_id', $ticket->payment->id)->first() : null;
                        @endphp
                        <tr class="even:bg-gray-100">
                            <td class="px-4 py-2 border border-gray-300">{{ $ticket->id }}</td>
                            <td class="px-4 py-2 border border-gray-300">
                                {{ $ticket->schedule->trip->route->origin ?? 'N/A' }} to 
                                {{ $ticket->schedule->trip->route->destination ?? 'N/A' }}
                            </td>
                            ...
                            <td class="px-4 py-2 border border-gray-300">
                                @if($refund)
                                    <span class="px-2 py-1 text-xs font-semibold rounded 
                                        ...
                                        ($refund->refund_status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">
